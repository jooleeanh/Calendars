require 'open-uri'
require 'nokogiri'
require_relative 'parser_event_BSO'
require_relative 'parser_calendar_BSO'
require "pry-byebug"
class ParserControllerBSO
  ...
  def initialize(calendar_url, view)
    ...
    @calendar = ParserCalendarBSO.new(nokodoc, @view, @calendar_url)
  end
  ...
  def scrape_all_events
    if @events_urls.empty?
      ...
      @events_urls.each_with_index do |event_url, index|
        ...
        event = ParserEventBSO.new(nokodoc, @view, event_url)
        event.parse_all
